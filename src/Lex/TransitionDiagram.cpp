...
TransitionDiagram *TransitionDiagram::removeEpsilonTransitions(bool inplace) {
    ...
    return removeEpsilonTransitionsInplace(new TransitionDiagram(this->getStartState(), std::vector<const NFAState *>(
            this->getEndStates().begin(), this->getEndStates().end())));
}
...
TransitionDiagram *TransitionDiagram::removeEpsilonTransitionsInplace(TransitionDiagram *transdig) {
    ...
    std::vector<const NFAState *> states = std::vector<const NFAState *>(transdig->getStates().begin(),
                                                                         transdig->getStates().end());
    for (auto state: states) {
 ...
 */
const NFAState *TransitionDiagram::mergeStates(
        std::map<std::set<const NFAState *>, std::map<char, std::set<const NFAState *>>> &new_table,
        const NFAState *start_state, std::unordered_set<const NFAState *> end_states,
        std::vector<const NFAState *> &new_end_states, std::vector<char> inputs) {
...
bool
TransitionDiagram::isEndStateNew(std::set<const NFAState *> states, std::unordered_set<const NFAState *> end_states) {
    for (auto state: states) {
