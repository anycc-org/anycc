...
TransitionDiagram *
DeterministicTransitionDiagramCreator::subsetConstruction(TransitionDiagram *transdig, bool inplace) {
    if (inplace) return subsetConstructionInplace(transdig);
    return subsetConstructionInplace(new TransitionDiagram(transdig->getStartState(), std::vector<const NFAState *>(
                                                                   transdig->getEndStates().begin(), transdig->getEndStates().end()), transdig->getTokens(),
                                                           transdig->getTokensPriority()));
}
...
TransitionDiagram *DeterministicTransitionDiagramCreator::subsetConstructionInplace(TransitionDiagram *transdig) {
    ...
    const NFAState *new_start_state = transdig->mergeStates(transdig, new_table, new_end_states,
